...
## * 用法
 ...
 - 先运行`crawler_cnstock.py`，`crawler_jrj.py`，`crawler_nbd.py`，`crawler_sina.py`，`crawler_stcn.py`这5个py文件，而且可能因为对方服务器没有响应而重复多次运行这几个文件才能抓取大量的历史数据
 - 接着运行`crawler_tushare.py`从Tushare获取基本信息和股票价格
 - 最后运行main.py文件，main.py中有4个步骤，除了第1步初始化外，其他几步最好单独运行
